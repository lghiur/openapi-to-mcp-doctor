...
import type { Behavior, DirectionResult, PrContext, ReportDelta } from './gh/types'
...
/** Scan options the base, head, and fix scans must share verbatim so their findings compare. */
interface SharedScanOptions {
 ...
 * finding is new.
 */
async function runBaseScan(args: {
  workspace: string
  baseRef: string
  /** Path of the spec ON THE BASE BRANCH (the pre-rename path when renamed). */
  basePathOfSpec: string
  scratchDir: string
  scan: SharedScanOptions
}): Promise<{ baseReport: AnalysisReport | undefined; baseScanDegraded: boolean }> {
  const shownBase = await git(args.workspace, [
    'show',
    `origin/${args.baseRef}:${args.basePathOfSpec}`,
  ])
  if (shownBase.exitCode !== 0) return { baseReport: undefined, baseScanDegraded: false }

  const baseSpecPath = path.join(args.scratchDir, `base-${path.basename(args.basePathOfSpec)}`)
  await writeFile(baseSpecPath, shownBase.stdout)
  const baseScan = await runScan({ specPath: baseSpecPath, json: true, ...args.scan })
  ...
  // it and mark the delta as unreliable in the sticky comment.
  ...
  return {
    baseReport:
      baseScan.exitCode !== 2 ? (JSON.parse(baseScan.stdout) as AnalysisReport) : undefined,
    baseScanDegraded: baseFailures.length > 0,
  }
}

/**
 * Everything the fix-pr level produced, threaded to the sticky comment:
 * `runFixPass` fills `patched`/`appliedFixCount`; `publishOrCloseFixPr` fills
 * `fixPr`/`blocked`. All fields stay unset below fix-pr behavior.
 */
interface FixPrOutcome {
  /** The patched spec content; undefined when the fix pass was skipped or failed. */
  patched?: string
  appliedFixCount?: number
  fixPr?: { url: string; number: number }
  blocked?: 'creation-forbidden' | 'permission-denied'
}
 ...
 */
async function runFixPass(args: {
  specPath: string
  scratchDir: string
  ...
  const { specPath, fixScope, confidenceThreshold } = args
  const selection = fixScope === 'pr' ? selectionFromFindings(args.newFindings) : undefined
  if (fixScope === 'pr' && selection === undefined) {
    ...
    // undefined, so the caller's obsolete-fix-PR close path takes over.
    process.stdout.write(
  ...
  const patchedPath = path.join(args.scratchDir, `patched-${path.basename(specPath)}`)
  ...
  const outcome: FixPrOutcome = {}
  if (fixScan.exitCode !== 2) {
    outcome.patched = await readFile(patchedPath, 'utf8').catch(() => undefined)
    outcome.appliedFixCount = parseAppliedFixCount(fixScan.stdout)
  }
  if (confidenceThreshold === 'low') {
    process.stdout.write(
      '::warning ::MCP Doctor AGGRESSIVE MODE — LOW-confidence fixes (including spec/code ' +
 ...
 */
async function publishOrCloseFixPr(
  octokit: Octokit,
  args: {
    ctx: PrContext
    specPath: string
...
): Promise<Pick<FixPrOutcome, 'fixPr' | 'blocked'>> {
  const { ctx, patched, confidenceThreshold, fixScope } = args
  if (patched === undefined || patched === args.headSpecText) {
    ...
    if (outcome === 'closed') {
      process.stdout.write('MCP Doctor: no fixes needed on this push — closed the stale fix PR.\n')
    }
    return {}
  }
  try {
    const fixPr = await ensureStackedFixPr(stackedPrApi(octokit), {
      owner: ctx.owner,
      repo: ctx.repo,
      sourceBranch: ctx.headRef,
    ...
    if (isPrCreationForbidden(error)) {
      ...
      return { blocked: 'creation-forbidden' }
    }
...
/** Sticky comment body: the rendered report plus any degradation/aggressive-mode notes. */
function stickyCommentBody(args: {
  delta: ReportDelta
  direction: DirectionResult
  report: AnalysisReport
  behavior: Behavior
  fix: FixPrOutcome
  skippedInline: number | undefined
  fixScope: 'pr' | 'full'
  confidenceThreshold: ConfidenceThreshold
  agentFailures: string[]
  baseScanDegraded: boolean
}): string {
  const { behavior, fix, confidenceThreshold, agentFailures } = args
  let body = renderStickyComment({
    ...
    behavior,
    fixPr: fix.fixPr,
    appliedFixCount: fix.appliedFixCount,
    skippedInline: args.skippedInline,
  ...
  if (fix.blocked === 'creation-forbidden') {
    body +=
  ...
  } else if (fix.blocked === 'permission-denied') {
    body +=
...
async function runPrMode(specPath: string, ctx: PrContext): Promise<never> {
  ...
  const scan: SharedScanOptions = {
    mcpVersion: input('mcp-version'),
    mismatchMode: choiceInput('mismatch-mode', MISMATCH_VALUES) ?? 'flag',
    routePaths,
    ai,
  }

  const { baseReport, baseScanDegraded } = await runBaseScan({
    workspace,
    baseRef: ctx.baseRef,
    basePathOfSpec: specOldPath ?? specPath,
    scratchDir,
    scan,
  })
  ...
  const headScan = await runScan({ specPath, json: true, ...scan, cache: true })
  ...
  if (behaviorAtLeast(behavior, 'comment') && token !== undefined) {
    ...
    if (behavior === 'fix-pr') {
      fix = {
        ...fix,
        ...(await publishOrCloseFixPr(octokit, {
          ctx,
          specPath,
          headSpecText,
          patched: fix.patched,
